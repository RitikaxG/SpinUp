...
async function listFilesInS3(sourcePrefix: string, continuationToken? : string){
...
async function storeFilesInVM(sourcePrefix: string){
...
async function copyS3Folder(sourcePrefix : string, destinationPrefix : string, continuationToken? : string) : Promise<void>{
    try{
        const listedObjects = await listFilesInS3(sourcePrefix);
        ...
        console.error(`Error copying folder in S3`);
    }
}   

// storeFilesInVM("base-app");
// copyS3Folder("base-app","projects/projectAlpha/base-app");